...
main = do
    ...
    print $ isSorted [-1, -2, -3, -4, -5, -6] == True
    print $ isSorted [-1, 1, 1, 0, 2, 3, 5] ==False
    --my test
...
isSorted [x] = True
isSorted [x, y] =True
isSorted (x:y:z:xs) = isTripleSorted x y z && isSorted(y:z:xs)

isTripleSorted :: (Ord a) => a -> a -> a -> Bool
isTripleSorted x y z = (x >= y && y >= z) || (x <= y && y <= z) 